...
export class RabbitMQController {
    ...
    constructor(private readonly urlAccessCounterConsumerService: UrlAccessCounterConsumerService) {}

    /**
     * Processes incoming messages on the queue.
     * @param data The payload of the message containing access counter data.
    ...
    public async execute(@Payload() data: IAccessCounterMessage, @Ctx() context: RmqContext): Promise<void> {
        ...
        try {
            await this.urlAccessCounterConsumerService.execute(shortCode);
            channel.ack(message);
        } catch (error) {
